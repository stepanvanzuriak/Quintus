const quintusTouch = function (Quintus) {
  Quintus.Touch = function (Q) {
    ...
    Q.Evented.extend("TouchSystem", {
      ...
      normalizeTouch(touch, stage) {
        ...
        this.touchPos.p.oy = this.touchPos.p.py =
          (posY / Q.cssHeight) * Q.height;
      ...
      touch(e) {
        ...
        for (let i = 0; i < touches.length; i++) {
          for (let stageIdx = 0; stageIdx < touchStage.length; stageIdx++) {
            ...
            const stage = Q.stage(touchStage[stageIdx]);

            if (!stage) {
              continue;
            }

            const touchIdentifier = touch.identifier || 0;
            const pos = this.normalizeTouch(touch, stage);

            stage.regrid(pos, true);
            const col = stage.search(pos, touchType);
            var obj;
      ...
      drag(e) {
        ...
        for (let i = 0; i < touches.length; i++) {
          ...
          const active = this.activeTouches[touchIdentifier];
...
if (window.Quintus) {
  quintusTouch(window.Quintus);
}

export default quintusTouch;
