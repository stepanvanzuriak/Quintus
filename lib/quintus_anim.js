const quintusAnim = function (Quintus) {
      ...
      step(dt) {
        ...
        for (property in this.start) {
          if (!Q._isUndefined(this.p[property])) {
            this.p[property] =
              this.start[property] + this.diff[property] * location;
          }
        }
      ...
      extend: {
        ...
        chain(properties, duration, easing, options) {
          ...
          if (tweenCnt > 0) {
            ...
            options = options || {};
            options.delay =
              lastTween.duration - lastTween.time + lastTween.delay;
          }
